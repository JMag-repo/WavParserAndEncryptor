...
namespace Header
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            RSA.KeyGenerator(1123, 1237);
            HeaderReader.FIND_INFO(path);
            HeaderReader.infochunk.ShowInfoChunk();
            



            /*BigInteger Data = new BigInteger(HeaderReader.data);
            var enc = RSA.EncryptData(HeaderReader.data);
            var dec = RSA.DecryptData(enc);
